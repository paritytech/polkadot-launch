...
async function registerAssetToParachain(
	...
	const { events: eventsRegister } = await createBlockWithExtrinsicParachain(
		...
		parachainApi.tx.sudo.sudo(
			parachainApi.tx.assetManager.registerAsset(
				assetLocation,
				assetMetadata,
				new BN(1)
			)
	...
	const { events } = await createBlockWithExtrinsicParachain(
		...
		sudoKeyring,
		parachainApi.tx.sudo.sudo(
			parachainApi.tx.assetManager.setAssetUnitsPerSecond(assetId, 0)
		)
	);
...
describeParachain(
	...
	(context) => {
		it.skip("should be able to receive an asset from relay", async function () {
			...
			// PARACHAINS
			// registerAsset
			const { events, assetId } = await registerAssetToParachain(
				parachainOne,
				alicePara
			);
			...
			expect((registeredAsset.toHuman() as { owner: string }).owner).to.eq(
				palletId
			);
				...
				relayOne.tx.xcmPallet.reserveTransferAssets(
					{
						V1: {
							parents: new BN(0),
							interior: { X1: { Parachain: new BN(1000) } },
						},
					},
					{
						V1: {
							...
							interior: {
								X1: {
									AccountKey20: { network: "Any", key: alicePara.addressRaw },
								},
							},
						},
					},
			...
			expect(
				(
					await parachainOne.query.assets.account(assetId, alicePara.addressRaw)
				).toHuman().balance === "1,000,000,000,000,000"
			).to.eq(true);
		});
	}
);
